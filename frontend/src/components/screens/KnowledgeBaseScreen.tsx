...
export const KnowledgeBaseScreen: React.FC<KnowledgeBaseScreenProps> = ({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Manage files used to generate grounded answers for your inquiries.
          </p>
        </div>

        <button
          onClick={() => setShowUploadModal(true)}
          className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-colors"
        >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="border-2 border-dashed border-slate-200 hover:border-indigo-400 bg-slate-50/50 rounded-xl p-6 text-center transition-colors">
            ...
            <p className="text-xs font-medium text-slate-700">
              Drop a PDF document here or browse
            </p>
            ...
            <label
              ...
              className="mt-4 inline-block px-3.5 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-medium text-slate-700 shadow-sm cursor-pointer hover:bg-slate-50"
            >
              Choose File
            </label>
          </div>
          ...
          {uploadEvents.length > 0 && (
            <div className="space-y-1.5 pt-2">
              {uploadEvents.map((evt) => (
                <div
                  key={evt.seq}
                  className="flex items-center justify-between text-xs text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg"
                >
                  <span className="font-medium">{evt.label}</span>
                  <span className="text-[10px] text-slate-400">{evt.t_ms}ms</span>
                </div>
              ))}
            </div>
          )}
      ...
      {/* Document Library Table / Empty State */}
        ...
        {documents.length === 0 ? (
          <div className="py-16 text-center text-slate-400 px-4">
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">
              Add PDF files to your document library to enable grounded answers during inquiries.
            </p>
            <button
              onClick={() => setShowUploadModal(true)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 text-[11px] font-medium text-slate-500 uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-slate-100">
                {documents.map((doc) => (
                  <tr key={doc.doc_id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-600">{doc.chunks}</td>
